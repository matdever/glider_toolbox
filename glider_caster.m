function cast_struct = glider_caster(var,depth,varargin)

% glider_caster  Separates upcasts from downcasts in a glider mission
%===================================================================
%
% USAGE:  cast_struct = glider_caster(var,depth,varargin);
%
...
for numvar = 1:N % For each variable
    ...
    if max(finish_down-start_down) > threshold/nanmean(abs(diff(depthvar)))
        ...
        cast_struct.downcast_depth{1,numvar} = NaN*ones(max(finish_down - start_down)+1,length(start_down));
